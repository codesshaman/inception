...
Разница в том, что ARG с параметрами задаёт переменную окружения с переданным параметром, а ARG без параметров слущает docker-compose и берёт параметр из такой же переменной в docker-compose.
...
Если мы хотим видеть наш сайт, не забываем указывать протокол https, ведь если мы раскомментируем наш редирект в конфиге nginx, мы уже не увидим его через хостовый браузер (перекинет на 42.fr - адрес).
...
Так же во время экзамена можно раскомментировать 301-й редирект в конфиге nginx и пересобрать проект, чтобы показать редиректы с http на https.
