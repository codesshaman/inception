...
Часто люди встречаются со следующей ошибкой: они пытаются запустить GUI через терминал, и теряются, когда не видят результата. Но терминальная сессия не предназначена для вывода графики, она лишь предоставляет текстовую оболочку для обмена данными.

Чтобы такой ошибки не возникало в дальнейшем, предлагаю сразу же её сэмулировать. Попытаемся запустить наш GUI в терминале следующей командой:

```sudo startx```

Как видим, наша графика успешно запустилась, но... Мы её не увидим:

![установка сертификата](media/install_certificate/step_11.png)

Всё потому, что терминал общается с пользователями при помощи текста и не предназначен для отрисовки графики. Он сделал всё что мог - вывел нам лог об успешном запуске графического окружения.

Графика же запустилась в окошке запущенной в virtualbox системы, и мы можем использовать её только там.

Однако если мы завершим сессию "иксов" в терминале, нажав ```Ctrl + C```, то и графика в виртуалке сразу же упадёт. Так работают "иксы" - если они запущены в терминале, надо открывать окно системы в virtualbox и работать в ней. А ещё проще залогиниться и запустить графику с браузером в virtualbox, а команды выполнять в терминале.
